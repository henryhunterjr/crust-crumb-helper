-- P1: wire the real Bread Authority into the content library.
-- Tag each content row with where it came from so the Bread Authority sync can
-- replace exactly its own rows on each run, never touching hand-added rows.
