...
function daysSince(dateStr: string | null): number { if (!dateStr) return Infinity; const d = new Date(dateStr); if (isNaN(d.getTime())) return Infinity; return Math.floor((Date.now() - d.getTime()) / 86_400_000); }
...
async function generateDm(member: any, outreachType: string) {
  return fetch(`${SUPABASE_URL}/functions/v1/generate-dm`, {
    method: "POST",
...
serve(async (req) => {
  ...
  try {
    ...
    for (const member of toProcess) {
      ...
      if (res.status === 429 || res.status === 402) { stopped = true; break; }
      if (!res.ok) { failed++; continue; }
      const data = await res.json();
      const message: string = data.message || "";
      if (!message.trim()) { failed++; continue; }
      results.push({ member: member.skool_name, type: outreachType, joinedDaysAgo: daysSince(member.join_date),
      ...
      if (!dryRun) {
        const { error: insErr } = await supabase.from("outreach_messages").insert({
          member_id: member.id, member_name: member.skool_name, message_type: outreachType,
          message_text: message, status: "generated", priority: isNew ? "high" : "medium",
      ...
      drafted++; if (isNew) welcomed++; else checkedIn++;
    }
    if (!dryRun && drafted > 0) {
      await supabase.from("activity_feed").insert({ activity_type: "auto_welcome",
    ...
    return json({ status: "completed", dryRun, candidates: candidates.length, newJoiners: newJoiners.length,
      backlogConsidered: backlog.length, drafted, welcomed, checkedIn, failed, stoppedOnRateLimit: stopped,
